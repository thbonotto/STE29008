    ...
    if (_parity == 0)
    {
      ...
      if ((!_gprmc_only) || (_gprmc_tag == 6))
      {
        ...
        if (_gprmc_tag == 6)
        {
          ...
          res_nUTCHour = _decimal(&_term[1][0]) * 10 + _decimal(&_term[1][1]);
          res_nUTCMin = _decimal(&_term[1][2]) * 10 + _decimal(&_term[1][3]);
          res_nUTCSec = _decimal(&_term[1][4]) * 10 + _decimal(&_term[1][5]);
...
float NMEA::degrees(float radians)
{
	float degrees=(radians*180)/pi;
	return degrees;
}

float NMEA::radians(float degrees)
{
...
float NMEA::initial_course(float lat1, float long1, float lat2, float long2)
{
  ...
  if (a2 < 0.0)
  {
    a2 += 2*pi;     // modulo operator doesn't seem to work on floats
  }
  return degrees(a2);
}
